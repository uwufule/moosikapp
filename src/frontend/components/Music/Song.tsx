...
const DefaultCoverSvg = styled.svg.attrs({ viewBox: '0 0 24 24' })`
  width: 32px;
  height: 32px;
...
const Song = ({
  ...
  const songList = useSelector<RootState, ISong[]>(
    (state) => state.player.songList,
  );

  const currentSong = useSelector<RootState, SongDetails | null>(
      ...
      <Cover coverUrl={cover}>
        ...
        <PlayPauseButton
          ...
          onClick={() => {
            ...
            const songIndex = songList.findIndex((song) => song.uuid === uuid);
            dispatch(setCurrentSongIndex(songIndex));
          }}
        >
          <svg viewBox="0 0 24 24">
            {(playing && currentSong?.uuid === uuid) ? <SvgPaths.Play /> : <SvgPaths.Pause />}
          </svg>
        </PlayPauseButton>
      </Cover>
      <TitleAndAuthor>
