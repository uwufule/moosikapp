...
const registerSceme = Joi.object({
  username: Joi.string()
    ...
    .max(64)
    .regex(/^\w+$/u)
    .error(new Error('Invalid username provided. Username must contain 1-64 letters or numbers.')),
  email: Joi.string()
    .required()
  ...
  password: Joi.string()
    .required()
    .min(8)
    .error(new Error('Invalid password provided. Username must be at least 8 symbols long.')),
});
...
export default (): RequestHandler => async (req: Request, res: Response) => {
  ...
  try {
    ...
    throw new BadRequest('An account with that email address and/or username already exists.');
  }
};
