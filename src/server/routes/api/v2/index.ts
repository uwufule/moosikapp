...
export default () => {
  ...
  // login
  router.post('/login', withAsyncErrorHandler(validateContentType('application/json'), login()));

  // refresh tokens
  router.post('/login/refresh', withAsyncErrorHandler(refresh()));

  // logout
  router.post('/logout', withAsyncErrorHandler(checkAuth(), logout()));

  // register
  router.post(
    '/register',
    withAsyncErrorHandler(validateContentType('application/json'), register()),
  );
  ...
  router.get('/users/:username', withAsyncErrorHandler(checkAuth(), users()));

  // get songs
  router.get('/songs', withAsyncErrorHandler(checkAuth(), Songs.getSongs()));
  ...
  // get favorite songs
  router.get('/favorites', withAsyncErrorHandler(checkAuth(), Favorites.getFavoriteSongs()));

  // add song to favorites
  router.post(
    '/favorites/:songId',
    withAsyncErrorHandler(checkAuth(), Favorites.addSongToFavorite()),
  );
  ...
  // status
  router.get('/status', withAsyncErrorHandler(status()));
